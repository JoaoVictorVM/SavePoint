...
export function FilterPairRow({
  ...
  return (
    <div className="p-4 rounded-[16px] border border-[var(--color-border)] bg-[var(--color-bg-surface)] space-y-3">
      {/* Header: label + remove */}
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wide">Filtro</span>
        {canRemove && (
          <button
            type="button"
            onClick={onRemove}
            className="p-1 rounded text-[var(--color-text-muted)] hover:text-[var(--color-error)] hover:bg-[var(--color-error)]/5 transition-colors cursor-pointer"
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-xs text-[var(--color-text-muted)]">Filtro 1</label>
        <select
          value={pair.category}
          onChange={(e) => handleCategoryChange(e.target.value as FilterCategory | "")}
          className="w-full h-10 px-3 rounded-full border border-[var(--color-border)] text-sm bg-[var(--color-bg-elevated)] text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] focus:border-transparent"
        >
      ...
      {pair.category && (
        <div className="flex flex-col gap-1.5">
          <label className="text-xs text-[var(--color-text-muted)]">Filtro 2</label>

          {isFavoritos ? (
            // Favoritos: single auto-checked, locked checkbox
            <div className="flex items-center gap-2 px-1 py-0.5">
              <div className="w-4 h-4 rounded border-2 border-[var(--color-accent)] bg-[var(--color-accent)] flex items-center justify-center shrink-0">
                <svg className="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M4.5 12.75l6 6 9-13.5" />
                </svg>
              </div>
              <span className="text-sm text-[var(--color-text-primary)]">Favorito</span>
            </div>
                    ...
                    <div
                      className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${
                        checked
                          ...
                          : "border-[var(--color-border)] bg-[var(--color-bg-elevated)] group-hover:border-[var(--color-accent)]"
                      }`}
